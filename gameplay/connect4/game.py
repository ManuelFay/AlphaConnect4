...
from alphaconnect4.interfaces.connect4 import Connect4Board, Connect4Tree
from alphaconnect4.interfaces.connect4.constants import YELLOW, RED, BLUE, SQUARESIZE, RADIUS, BLACK
from alphaconnect4.agents.base_agent import BaseAgent

from gameplay.connect4.visual_engine import VisualEngine


class Game:
    def __init__(self, agent0: BaseAgent = None, agent1: BaseAgent = None, enable_ui: bool = True):
        self.board = Connect4Board(board=None, turn=random.choice([0, 1]))
    ...
    def play(self):
        ...
        while not self.game_over:
            ...
            if self.board.turn == 0 and self.agent0 is not None:  # If it is the AI turn
                board = Connect4Tree(board=self.board.board, turn=self.board.turn)
                col = self.agent0.move(board)
                self.make_move((self.board.get_next_open_row(col), col))
                if self.visual_engine:
